...
function isRunData(data: unknown): data is RunData {
  return (
    ...
    'args' in data &&
    'env' in data &&
    Array.isArray((data as Record<string, unknown>).args) &&
...
class BunWorker {
  ...
  private async handleRun(requestId: string, data: unknown): Promise<void> {
    if (!isRunData(data)) {
      this.sendError(requestId, 'Invalid run data format');
      return;
    }
    const runData = data;
  ...
  private handleWatchOutput(data: string): void {
    ...
    if ((data.includes('Watching for changes') || data.includes('test run finished')) && this.currentRequestId) {
      const response: WorkerMessage = {
        type: 'result',
