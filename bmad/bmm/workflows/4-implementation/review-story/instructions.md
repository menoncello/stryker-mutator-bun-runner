...
{{result_story_list}}
  ...
  <step n="5.5" goal="Quality gates verification">
    ...
    <substep n="5.5.1" goal="TypeScript compilation verification">
      ...
      <action>Scan changed files for @ts-ignore/@ts-expect-error</action>
      <check>If found → flag as High Severity violation</check>
    </substep>
    ...
    <substep n="5.5.4" goal="Mutation testing verification">
      <action>Check if mutation testing was run (bun run test:mutate)</action>
      <check>If mutation score below 80% → flag as Medium Severity</check>
