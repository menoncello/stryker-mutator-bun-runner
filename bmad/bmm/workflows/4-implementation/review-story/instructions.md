...
Running in standalone mode - no progress tracking.</output>
  ...
  <step n="5.5" goal="Quality gates verification">
    <critical>Verify quality gates were enforced during development</critical>

    <substep n="5.5.1" goal="TypeScript compilation verification">
      <action>Run: bun run typecheck (or tsc --noEmit)</action>
      <check>ZERO TypeScript errors - flag as High Severity if found</check>
      <action>Scan changed files for 'any' type usage</action>
      <check>If 'any' types found → flag as High Severity violation (ADR-004)</check>
    </substep>

    <substep n="5.5.2" goal="ESLint compliance verification">
      <action>Run: bun run lint</action>
      <check>ZERO ESLint errors - flag as High Severity if found</check>
      <action>Scan changed files for eslint-disable comments</action>
      <check>If eslint-disable found → flag as High Severity violation</check>
      <action>Scan changed files for @ts-ignore or @ts-expect-error</action>
      <check>If @ts-ignore/@ts-expect-error found → flag as High Severity violation</check>
    </substep>

    <substep n="5.5.3" goal="Test execution and coverage verification">
      <action>Run: bun test</action>
      <check>100% test pass rate - flag as High Severity if failed</check>
      <action>Verify test coverage ≥80% (NFR009)</action>
      <check>If coverage below threshold → flag as Medium Severity</check>
    </substep>

    <substep n="5.5.4" goal="Mutation testing verification (when applicable)">
      <action>Check if mutation testing was run (bun run test:mutate)</action>
      <check>If mutation score below threshold → flag as Medium Severity</check>
      <action>Verify no mutation testing thresholds were lowered</action>
      <check>If thresholds lowered → flag as High Severity violation</check>
    </substep>

    <substep n="5.5.5" goal="Architecture compliance verification">
      <action>Scan changed files for architecture pattern compliance</action>
      <check>Sanitized logging (FR037) - flag violations as High Severity</check>
      <check>Resource cleanup in try/finally (FR027, FR028, FR038) - flag violations as High Severity</check>
      <check>Input validation (FR004, FR034) - flag violations as Medium Severity</check>
      <check>Actionable error messages (NFR025) - flag violations as Low Severity</check>
    </substep>

    <action>Aggregate all quality gate violations into Key Findings section</action>
    <action if="High Severity violations found">Set Outcome: Blocked</action>
    <action if="only Medium/Low Severity violations found">Set Outcome: Changes Requested</action>
  </step>
